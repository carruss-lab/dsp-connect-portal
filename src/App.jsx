...
const ADMIN_CREDS = {email:"admin@getdspconnect.com",password:"dspconnect2025"};
...
function calcScore(p,pipeline){
  ...
  const days=daysIn(p.start_date);
  if(days<=30&&(p.day30_aum||0)>=500000)s+=10;
  else if(days<=30&&(p.day30_aum||0)>=100000)s+=5;
...
function StatusPill({s}){
  const m={Active:[C.greenBg,C.green],Onboarding:[C.amberBg,C.amber],Inactive:[C.redBg,C.red],"Pending Setup":[C.bg2,C.muted],Lead:["#F1EFE8","#444441"],"Deposit Paid":[C.amberBg,C.amber],Review:[C.blueBg,C.blue],Approved:[C.tealBg,C.teal]};
  const[bg,fg]=m[s]||[C.bg2,C.muted];return <Pill label={s} bg={bg} fg={fg}/>;
}
function StatBox({label,value,sub,icon,green,blue}){
  return <div style={{background:C.bg2,borderRadius:10,padding:"14px 16px"}}>
    ...
    <div style={{fontSize:22,fontWeight:500,color:green?C.green:blue?C.blue:C.text}}>{value}</div>
...
function SectionHead({title,action}){return <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><h2 style={{margin:0,fontSize:18,fontWeight:500}}>{title}</h2>{action}</div>;}
...
const ADMIN_NAV=[
  ...
  {section:"FINANCE",items:[{key:"comp-tracking",icon:"currency-dollar",label:"Compensation"},{key:"stripe",icon:"credit-card",label:"Stripe payouts"}]},
  {section:"CONTENT",items:[{key:"content-mgmt",icon:"upload",label:"Content management"}]},
    ...
    <div style={{flex:1,padding:"8px 0"}}>
      {navGroups.map(g=><div key={g.section}>
        ...
        {g.items.map(n=><button key={n.key} onClick={()=>onSelect(n.key)} style={{display:"flex",alignItems:"center",gap:9,padding:"8px 14px",width:"100%",border:"none",background:active===n.key?C.bg2:"transparent",color:active===n.key?C.text:C.muted,cursor:"pointer",fontSize:13,fontWeight:active===n.key?500:400,textAlign:"left"}}>
          ...
          {n.badge&&<span style={{marginLeft:"auto",background:C.blue,color:"#fff",fontSize:10,borderRadius:10,padding:"1px 6px"}}>{n.badge}</span>}
        </button>)}
      </div>)}
    </div>
...
function Login({onLogin}){
  ...
  const attempt=async()=>{
    ...
    if(email.trim().toLowerCase()===ADMIN_CREDS.email&&pass===ADMIN_CREDS.password){setLoading(false);onLogin({role:"admin"});return;}
  ...
  return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg3,fontFamily:"inherit"}}>
    <div style={{width:400,background:C.bg,border:`0.5px solid ${C.border}`,borderRadius:16,padding:36}}>
      ...
      <div style={{marginTop:20,padding:14,background:C.bg2,borderRadius:10,fontSize:12,color:C.muted}}>
        <div style={{fontWeight:500,color:C.text,marginBottom:4}}>Admin: admin@getdspconnect.com</div>
        Partner logins are created by admin and shared directly with each managing partner.
      </div>
    </div>
  </div>;
}
...
function Onboarding({partner,onComplete}){
  ...
  const steps=[
    {title:"Cash compensation policy",icon:"currency-dollar",content:<div style={{fontSize:13,lineHeight:1.7}}>
      ...
      {[["Rate","10% of Qualified Ad Spend Under Management (AUM)"],["Payment schedule","Monthly, after advertiser funds collected by DSP Connect"],["Qualifying spend","Contracted, activated, invoiced, and collected"],["Non-qualifying","Unpaid invoices, chargebacks, refunds, disputes, write-offs"],["Recurring clients","Compensation continues on originated active accounts"],["No guarantee","Compensation is production-based only — no salary minimum"]].map(([k,v])=><div key={k} style={{display:"flex",gap:14,padding:"7px 0",borderBottom:`0.5px solid ${C.border}`}}><span style={{color:C.muted,minWidth:150,flexShrink:0}}>{k}</span><span>{v}</span></div>)}
      <label style={{display:"flex",alignItems:"center",gap:10,marginTop:16,cursor:"pointer"}}><input type="checkbox" checked={checks.comp} onChange={e=>setChecks(c=>({...c,comp:e.target.checked}))} style={{width:16,height:16}}/>I have read and understood the Cash Compensation Policy (Exhibit B)</label>
    </div>},
    {title:"Equity vesting policy",icon:"chart-pie",content:<div style={{fontSize:13,lineHeight:1.7}}>
      ...
      {[["Equity pool","40% of total LLC membership interests"],["Base grant","5.00% upon qualifying"],["Annual minimum","$5,000,000 AUM in first 12 months"],["Vesting cliff","1 year — no partial vesting before anniversary"],["Max first year","15.00% (5% base + 10% acceleration)"]].map(([k,v])=><div key={k} style={{display:"flex",gap:14,padding:"7px 0",borderBottom:`0.5px solid ${C.border}`}}><span style={{color:C.muted,minWidth:150,flexShrink:0}}>{k}</span><span>{v}</span></div>)}
      ...
      <label style={{display:"flex",alignItems:"center",gap:10,marginTop:14,cursor:"pointer"}}><input type="checkbox" checked={checks.equity} onChange={e=>setChecks(c=>({...c,equity:e.target.checked}))} style={{width:16,height:16}}/>I have read and understood the Equity Vesting Policy (Exhibit A)</label>
    </div>},
    {title:"Operating rules",icon:"list-check",content:<div style={{fontSize:13,lineHeight:1.7}}>
      ...
      {[["Non-compete",`${partner.non_compete_days||90} days post-termination`],["Trailing comp",TRAILING_OPTIONS.find(o=>o.value===(partner.trailing_option||"A"))?.label],["Account ownership","DSP Connect owns all advertiser accounts"],["Good standing","Vesting requires continuous compliance with all policies"],["IP","All platform materials remain property of DSP Connect"]].map(([k,v])=><div key={k} style={{display:"flex",gap:14,padding:"7px 0",borderBottom:`0.5px solid ${C.border}`}}><span style={{color:C.muted,minWidth:150,flexShrink:0}}>{k}</span><span>{v}</span></div>)}
      <label style={{display:"flex",alignItems:"center",gap:10,marginTop:14,cursor:"pointer"}}><input type="checkbox" checked={checks.ops} onChange={e=>setChecks(c=>({...c,ops:e.target.checked}))} style={{width:16,height:16}}/>I acknowledge and agree to the operating rules above</label>
      <label style={{display:"flex",alignItems:"center",gap:10,marginTop:10,cursor:"pointer"}}><input type="checkbox" checked={checks.noncompete} onChange={e=>setChecks(c=>({...c,noncompete:e.target.checked}))} style={{width:16,height:16}}/>I acknowledge the non-compete and trailing compensation terms</label>
    </div>},
    {title:"Digital signature",icon:"pencil",content:<div style={{fontSize:13,lineHeight:1.7}}>
      ...
      <div style={{marginTop:16,marginBottom:6,fontWeight:500,fontSize:13}}>Type your full legal name to sign</div>
  ...
  return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg3,padding:24}}>
    <div style={{width:"100%",maxWidth:640}}>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
        {step>0&&<button onClick={()=>setStep(s=>s-1)} style={{fontSize:13,padding:"8px 16px"}}>Back</button>}
        {step<steps.length-1
          ?<button onClick={()=>{if(canAdvance)setStep(s=>s+1);}} style={{fontSize:13,padding:"8px 20px",background:canAdvance?C.blue:"#ccc",color:"#fff",border:"none",borderRadius:8,cursor:canAdvance?"pointer":"not-allowed"}}>Continue →</button>
          :<button onClick={onComplete} disabled={!canAdvance} style={{fontSize:13,padding:"8px 20px",background:canAdvance?C.green:"#ccc",color:"#fff",border:"none",borderRadius:8,cursor:canAdvance?"pointer":"not-allowed"}}>Access my dashboard →</button>}
      </div>
    </div>
  </div>;
}
...
function PartnerApp({partner,onLogout}){
  ...
  useEffect(()=>{
    Promise.all([
      supabase.from("pipeline").select("*").eq("partner_id",partner.id),
      supabase.from("resources").select("*").order("created_at",{ascending:false}),
      supabase.from("tickets").select("*").eq("partner_id",partner.id).order("created_at",{ascending:false}),
    ]).then(([p,r,t])=>{
      setPipeline(p.data||[]);setResources(r.data||[]);setTickets(t.data||[]);setLoading(false);
    });
  },[partner.id]);

  const days=daysIn(partner.start_date);
  const monthly=(partner.monthly_spend||0)*REV_RATE;
  const equity=calcEquity(partner);
  const myPipeline=pipeline;
  const brandClients=myPipeline.filter(c=>c.type==="brand");
  const agencyClients=myPipeline.filter(c=>c.type==="agency");
  ...
  const navGroups=PARTNER_NAV.map(g=>({...g,items:g.items.map(i=>i.key==="pipeline"?{...i,badge:myPipeline.filter(c=>c.stage==="Review").length||null}:i)}));
    ...
    <div style={{flex:1,overflowY:"auto"}}>
      ...
      {view==="dashboard"&&<div style={{padding:24}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
          <StatBox label="Monthly AUM" value={fmtK(partner.monthly_spend||0)} sub="Managed spend" icon="chart-bar"/>
          <StatBox label="Monthly compensation" value={fmt$(monthly)} sub="10% of AUM" icon="currency-dollar" green/>
          <StatBox label="Active clients" value={myPipeline.filter(c=>c.stage==="Active").length} sub={`${myPipeline.length} total`} icon="building"/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
          <Card style={{padding:16}}>
            ...
            <div style={{background:C.blueBg,borderRadius:8,padding:"10px 12px",marginBottom:8}}><div style={{fontSize:12,fontWeight:500,color:C.blue,marginBottom:2}}>Welcome to DSP Connect</div><div style={{fontSize:12,color:C.blue}}>Your Managing Partner portal is now active. Complete your 90-day acceleration window to maximize equity.</div></div>
            <div style={{background:C.amberBg,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:12,fontWeight:500,color:C.amber,marginBottom:2}}>Reminder: 90-day windows</div><div style={{fontSize:12,color:C.amber}}>Missed acceleration windows are permanently forfeited.</div></div>
          </Card>
          <Card style={{padding:16}}>
        ...
        <Card style={{padding:16}}>
          <div style={{fontWeight:500,fontSize:14,marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>Pipeline summary<button onClick={()=>setView("pipeline")} style={{fontSize:12,color:C.blue,background:"none",border:"none",cursor:"pointer"}}>View all →</button></div>
          {myPipeline.length===0?<div style={{fontSize:13,color:C.muted,textAlign:"center",padding:"16px 0"}}>No clients yet. <button onClick={()=>setView("brand-clients")} style={{background:"none",border:"none",color:C.blue,cursor:"pointer",fontSize:13}}>Add your first client →</button></div>
          :PIPELINE_STAGES.map(stage=>{const inStage=myPipeline.filter(c=>c.stage===stage);if(!inStage.length)return null;
      ...
      {view==="brand-clients"&&<div style={{padding:24}}>
        <SectionHead title="Brand clients" action={<button onClick={()=>{setClientType("brand");setShowAddClient(true);}} style={{display:"flex",alignItems:"center",gap:6,fontSize:13,padding:"7px 14px"}}><i className="ti ti-plus" style={{fontSize:13}}/>Add brand client</button>}/>
        {showAddClient&&clientType==="brand"&&<Card style={{padding:16,marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[["Company name *","name","text"],["Contact name","contact_name","text"],["Email","email","email"],["Monthly spend ($)","monthly_spend","number"]].map(([label,field,type])=><div key={field}><label style={{fontSize:12,color:C.muted,display:"block",marginBottom:4}}>{label}</label><input type={type} value={newClient[field]} onChange={e=>setNewClient(c=>({...c,[field]:e.target.value}))} style={{width:"100%",boxSizing:"border-box"}}/></div>)}
          </div>
          <div style={{display:"flex",gap:8,marginTop:12}}><button onClick={()=>addClient("brand")} style={{fontSize:13,padding:"7px 14px",background:C.blue,color:"#fff",border:"none",borderRadius:8,cursor:"pointer"}}>Add client</button><button onClick={()=>setShowAddClient(false)} style={{fontSize:13,padding:"7px 14px"}}>Cancel</button></div>
        </Card>}
        {brandClients.length===0?<EmptyState icon="building" title="No brand clients yet" sub="Add your first direct advertiser client." cta="Add brand client" onCta={()=>{setClientType("brand");setShowAddClient(true);}}/>
        :<Card style={{overflow:"hidden"}}>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"10px 16px",background:C.bg2,borderBottom:`0.5px solid ${C.border}`,fontSize:11,fontWeight:500,color:C.muted}}><span>Company</span><span>Monthly spend</span><span>Stage</span><span>Added</span></div>
          {brandClients.map((c,i)=><div key={c.id} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"12px 16px",borderBottom:i<brandClients.length-1?`0.5px solid ${C.border}`:"none",alignItems:"center"}}>
            <div><div style={{fontWeight:500,fontSize:13}}>{c.name}</div><div style={{fontSize:11,color:C.muted}}>{c.contact_name}</div></div>
            <div style={{fontSize:13,fontWeight:500,color:C.green}}>{fmtK(c.monthly_spend||0)}</div>
      ...
      {view==="agency-clients"&&<div style={{padding:24}}>
        <SectionHead title="Agency clients" action={<button onClick={()=>{setClientType("agency");setShowAddClient(true);}} style={{display:"flex",alignItems:"center",gap:6,fontSize:13,padding:"7px 14px"}}><i className="ti ti-plus" style={{fontSize:13}}/>Add agency client</button>}/>
        {showAddClient&&clientType==="agency"&&<Card style={{padding:16,marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:12}}><button onClick={()=>addClient("agency")} style={{fontSize:13,padding:"7px 14px",background:C.blue,color:"#fff",border:"none",borderRadius:8,cursor:"pointer"}}>Add agency</button><button onClick={()=>setShowAddClient(false)} style={{fontSize:13,padding:"7px 14px"}}>Cancel</button></div>
        </Card>}
        ...
        :<Card style={{overflow:"hidden"}}>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"10px 16px",background:C.bg2,borderBottom:`0.5px solid ${C.border}`,fontSize:11,fontWeight:500,color:C.muted}}><span>Agency</span><span>Monthly spend</span><span>Stage</span><span>Added</span></div>
          {agencyClients.map((c,i)=><div key={c.id} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"12px 16px",borderBottom:i<agencyClients.length-1?`0.5px solid ${C.border}`:"none",alignItems:"center"}}>
            <div><div style={{fontWeight:500,fontSize:13}}>{c.name}</div><div style={{fontSize:11,color:C.muted}}>{c.contact_name}</div></div>
            <div style={{fontSize:13,fontWeight:500,color:C.green}}>{fmtK(c.monthly_spend||0)}</div>
            <StatusPill s={c.stage}/>
            <div style={{fontSize:12,color:C.muted}}>{(c.created_at||"").slice(0,10)}</div>
      ...
      {view==="pipeline"&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 20px",fontSize:18,fontWeight:500}}>Pipeline</h2>
        {["brand","agency"].map(type=><div key={type} style={{marginBottom:28}}>
          <div style={{fontWeight:500,fontSize:14,marginBottom:12,display:"flex",alignItems:"center",gap:8}}><i className={`ti ti-${type==="brand"?"building":"briefcase"}`} style={{fontSize:15,color:C.blue}}/>{type==="brand"?"Brand clients":"Agency clients"}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
            {PIPELINE_STAGES.map(stage=>{const inStage=myPipeline.filter(c=>c.type===type&&c.stage===stage);
              return <div key={stage}>
                ...
                <div style={{minHeight:60,background:C.bg2,borderRadius:8,padding:6,display:"flex",flexDirection:"column",gap:6}}>
                  {inStage.map(c=><div key={c.id} style={{background:C.bg,border:`0.5px solid ${C.border}`,borderRadius:6,padding:"8px 10px"}}>
                    ...
                    <div style={{fontSize:11,color:C.muted,marginBottom:6}}>{fmtK(c.monthly_spend||0)}/mo</div>
      ...
      {view==="compensation"&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 4px",fontSize:18,fontWeight:500}}>Compensation center</h2>
        <div style={{fontSize:12,color:C.muted,marginBottom:20}}>10% of Qualified AUM · Cash Compensation Policy</div>
        <div style={{marginBottom:12,fontWeight:500,fontSize:13,color:C.muted}}>Current month</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
          <StatBox label="Brand revenue" value={fmtK(brandClients.filter(c=>c.stage==="Active").reduce((s,c)=>s+(c.monthly_spend||0),0))} icon="building" green/>
          <StatBox label="Agency revenue" value={fmtK(agencyClients.filter(c=>c.stage==="Active").reduce((s,c)=>s+(c.monthly_spend||0),0))} icon="briefcase" green/>
          <StatBox label="Revenue share (10%)" value={fmt$(monthly)} icon="currency-dollar" green/>
          <StatBox label="Annual run rate" value={fmtK(monthly*12)} icon="trending-up" green/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
          <StatBox label="YTD earnings (est.)" value={fmt$(monthly*Math.max(1,Math.floor(days/30)))} icon="calendar" green/>
          <StatBox label="Lifetime (est.)" value={fmt$(monthly*Math.max(1,Math.floor(days/30)))} icon="trophy" green/>
          <StatBox label="Monthly AUM" value={fmtK(partner.monthly_spend||0)} icon="chart-bar"/>
        </div>
      </div>}

      {view==="equity"&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 4px",fontSize:18,fontWeight:500}}>Ownership center</h2>
        <div style={{fontSize:12,color:C.muted,marginBottom:20}}>Equity Vesting, Performance & Acceleration Policy</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
          ...
          <StatBox label="Next milestone" value={equity.qualifies?"Max achieved":fmtK(5000000-(partner.annual_aum||0))} sub={equity.qualifies?"":"to qualify"} icon="flag"/>
          <StatBox label="Equity value" value="Contact admin" icon="currency-dollar"/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <Card style={{padding:16}}>
            <div style={{fontWeight:500,fontSize:14,marginBottom:16}}>Equity breakdown</div>
            {[{label:"Base grant (5%)",pct:equity.base,max:5,color:C.blue},{label:"Day 30 acceleration",pct:equity.a30,max:3,color:C.amber},{label:"Day 60 acceleration",pct:equity.a60,max:3,color:C.green},{label:"Day 90 acceleration",pct:equity.a90,max:4,color:C.purple}].map(r=><div key={r.label} style={{marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:4}}><span style={{color:C.muted}}>{r.label}</span><span style={{fontWeight:500}}>{equity.qualifies?r.pct.toFixed(2):0}%</span></div>
              <ProgressBar pct={equity.qualifies?(r.pct/r.max)*100:0} color={r.color}/>
            </div>)}
            <div style={{borderTop:`0.5px solid ${C.border}`,marginTop:12,paddingTop:12,display:"flex",justifyContent:"space-between",fontSize:14,fontWeight:500}}><span>Total</span><span style={{color:C.purple}}>{equity.total.toFixed(2)}% / 15.00%</span></div>
          </Card>
          <Card style={{padding:16}}>
            <div style={{fontWeight:500,fontSize:14,marginBottom:14}}>Equity documents</div>
      ...
      {["training","sops","documents"].includes(view)&&<div style={{padding:24}}>
        ...
        {resources.filter(r=>r.category===view).length===0
          ...
          :<div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12}}>
            {resources.filter(r=>r.category===view).map(r=><Card key={r.id} style={{padding:14}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}><i className={`ti ti-${r.type==="video"?"player-play":"file-text"}`} style={{fontSize:18,color:C.blue}}/>
              ...
              {r.description&&<div style={{fontSize:12,color:C.muted,marginTop:8}}>{r.description}</div>}
            </Card>)}
          </div>}
      </div>}

      {view==="support"&&<div style={{padding:24}}>
        <SectionHead title="Help desk" action={<button onClick={()=>setShowTicket(true)} style={{display:"flex",alignItems:"center",gap:6,fontSize:13,padding:"7px 14px"}}><i className="ti ti-plus" style={{fontSize:13}}/>Submit ticket</button>}/>
        {showTicket&&<Card style={{padding:16,marginBottom:16}}>
          ...
          <div style={{marginBottom:10}}><label style={{fontSize:12,color:C.muted,display:"block",marginBottom:4}}>Details</label><input value={newTicket.details} onChange={e=>setNewTicket(t=>({...t,details:e.target.value}))} placeholder="Full description…" style={{width:"100%",boxSizing:"border-box"}}/></div>
          <div style={{display:"flex",gap:8}}><button onClick={submitTicket} style={{fontSize:13,padding:"7px 14px",background:C.blue,color:"#fff",border:"none",borderRadius:8,cursor:"pointer"}}>Submit</button><button onClick={()=>setShowTicket(false)} style={{fontSize:13,padding:"7px 14px"}}>Cancel</button></div>
        </Card>}
        ...
        :<Card style={{overflow:"hidden"}}>
          {tickets.map((t,i,arr)=><div key={t.id} style={{padding:"12px 16px",borderBottom:i<arr.length-1?`0.5px solid ${C.border}`:"none"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}><Pill label={t.type} bg={C.blueBg} fg={C.blue}/><span style={{fontWeight:500,fontSize:13,flex:1}}>{t.subject}</span><Pill label={t.status} bg={t.status==="Open"?C.amberBg:C.greenBg} fg={t.status==="Open"?C.amber:C.green}/><span style={{fontSize:12,color:C.muted}}>{(t.created_at||"").slice(0,10)}</span></div>
            {t.details&&<div style={{fontSize:12,color:C.muted,marginTop:4}}>{t.details}</div>}
          </div>)}
        </Card>}
      </div>}
    </div>
...
function AdminApp({onLogout}){
  ...
  const[newResource,setNewResource]=useState({title:"",type:"pdf",category:"training",description:""});
  ...
  useEffect(()=>{
    Promise.all([
      ...
      setPartners(p.data||[]);setPipeline(pi.data||[]);setResources(r.data||[]);setTickets(t.data||[]);setLoading(false);
    });
  },[]);
  ...
  const addResource=async()=>{
    ...
    setNewResource({title:"",type:"pdf",category:"training",description:""});setShowAddResource(false);
  };
  ...
  const navGroups=ADMIN_NAV.map(g=>({...g,items:g.items.map(i=>({...i,badge:i.key==="approvals"?pendingApprovals.length||null:i.key==="admissions"?partners.filter(p=>p.status==="Pending Setup").length||null:i.key==="slack"?slackMsgs.length||null:null}))}));
    ...
    <div style={{flex:1,overflowY:"auto"}}>
      ...
      {view==="dashboard"&&<div style={{padding:24}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
          ...
          <StatBox label="Total monthly AUM" value={fmtK(totalAUM)} icon="chart-bar"/>
          <StatBox label="Monthly comp due" value={fmt$(totalComp)} sub="10% of AUM" icon="currency-dollar" green/>
          <StatBox label="Pending review" value={pendingApprovals.length} sub="Client approvals" icon="clock"/>
        </div>
        ...
        :<Card style={{overflow:"hidden"}}>
          ...
          {partners.map((p,i)=>{const eq=calcEquity(p);return <div key={p.id} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderBottom:i<partners.length-1?`0.5px solid ${C.border}`:"none"}}>
            <Avatar name={p.name}/><div style={{flex:1,minWidth:0}}><div style={{fontWeight:500,fontSize:13}}>{p.name}{p.agency?` · ${p.agency}`:""}</div><div style={{fontSize:11,color:C.muted}}>{p.vertical} · Day {daysIn(p.start_date)}</div></div>
            <StatusPill s={p.status}/>
            <div style={{textAlign:"right",minWidth:90}}><div style={{fontSize:13,fontWeight:500}}>{fmtK(p.monthly_spend||0)}</div><div style={{fontSize:11,color:C.muted}}>AUM/mo</div></div>
            <div style={{textAlign:"right",minWidth:75}}><div style={{fontSize:13,fontWeight:500,color:C.green}}>{fmt$((p.monthly_spend||0)*REV_RATE)}</div><div style={{fontSize:11,color:C.muted}}>comp/mo</div></div>
      ...
      {view==="admissions"&&<div style={{padding:24}}>
        <SectionHead title="Partner admissions" action={<button onClick={()=>setShowAdd(s=>!s)} style={{fontSize:13,padding:"8px 16px",background:showAdd?"transparent":C.blue,color:showAdd?C.text:"#fff",border:showAdd?`0.5px solid ${C.border}`:"none",borderRadius:8,cursor:"pointer"}}>{showAdd?"Cancel":"+ New partner"}</button>}/>
        {showAdd&&<Card style={{padding:20,marginBottom:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div style={{gridColumn:"1/-1",paddingTop:12,borderTop:`0.5px solid ${C.border}`}}>
              <div style={{fontSize:12,color:C.muted,marginBottom:10,fontWeight:500}}>AUM — enter once spend is active, can update later</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:14}}>
            <button onClick={addPartner} disabled={saving} style={{fontSize:13,padding:"8px 20px",background:C.blue,color:"#fff",border:"none",borderRadius:8,cursor:"pointer"}}>{saving?"Saving…":"Create partner account"}</button>
            <button onClick={()=>{setShowAdd(false);setForm(blank);setFormErr("");}} style={{fontSize:13,padding:"8px 14px"}}>Cancel</button>
          </div>
        </Card>}
        {partners.filter(p=>p.status==="Pending Setup").length>0&&<div style={{marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:500,color:C.muted,marginBottom:10}}>Pending setup</div>
          {partners.filter(p=>p.status==="Pending Setup").map(p=><Card key={p.id} style={{padding:14,marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}><Avatar name={p.name}/><div style={{flex:1}}><div style={{fontWeight:500,fontSize:13}}>{p.name}{p.agency?` · ${p.agency}`:""}</div><div style={{fontSize:11,color:C.muted}}>{p.email}</div></div><StatusPill s={p.status}/>
            <button onClick={()=>updatePartner(p.id,{status:"Active"})} style={{fontSize:12,padding:"6px 12px",background:C.greenBg,color:C.green,border:`0.5px solid #C0DD97`,borderRadius:8,cursor:"pointer"}}>Activate</button></div>
          </Card>)}
        </div>}
      ...
      {view==="partner-mgmt"&&<div style={{padding:24}}>
        ...
        :partners.map(p=>{const eq=calcEquity(p);const open=selected?.id===p.id;
          return <Card key={p.id} style={{marginBottom:12,overflow:"hidden"}}>
            <div onClick={()=>setSelected(open?null:p)} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",cursor:"pointer",background:open?C.bg2:"transparent"}}>
              <Avatar name={p.name} size={42}/><div style={{flex:1,minWidth:0}}><div style={{fontWeight:500,fontSize:14}}>{p.name}{p.agency?` · ${p.agency}`:""}</div><div style={{fontSize:12,color:C.muted}}>{p.email} · {p.vertical} · Day {daysIn(p.start_date)}</div></div>
              <StatusPill s={p.status}/><Pill label={`${eq.total.toFixed(1)}% equity`} bg={C.purpleBg} fg={C.purple}/>
              <div style={{textAlign:"right",minWidth:90}}><div style={{fontSize:14,fontWeight:500}}>{fmtK(p.monthly_spend||0)}</div><div style={{fontSize:11,color:C.muted}}>AUM/mo</div></div>
              <div style={{textAlign:"right",minWidth:80}}><div style={{fontSize:14,fontWeight:500,color:C.green}}>{fmt$((p.monthly_spend||0)*REV_RATE)}</div><div style={{fontSize:11,color:C.muted}}>comp/mo</div></div>
            </div>
            {open&&<div style={{padding:16,borderTop:`0.5px solid ${C.border}`,background:C.bg2}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
                <StatBox label="Monthly AUM" value={fmtK(p.monthly_spend||0)} icon="chart-bar"/>
                <StatBox label="Annual AUM" value={fmtK(p.annual_aum||0)} icon="trending-up"/>
                <StatBox label="Equity" value={`${eq.total.toFixed(2)}%`} icon="chart-pie"/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
                {[{label:"Monthly AUM ($)",f:"monthly_spend"},{label:"Annual AUM ($)",f:"annual_aum"},{label:"Day 30 AUM ($)",f:"day30_aum"},{label:"Day 60 AUM ($)",f:"day60_aum"},{label:"Day 90 AUM ($)",f:"day90_aum"}].map(({label,f})=><div key={f}><label style={{fontSize:11,color:C.muted,display:"block",marginBottom:4}}>{label}</label><input type="number" defaultValue={p[f]||0} onBlur={e=>updatePartner(p.id,{[f]:parseFloat(e.target.value)||0})} style={{width:"100%",boxSizing:"border-box",fontSize:13}}/></div>)}
              </div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <button onClick={()=>sendSlack("#partner-revenue",`💰 ${p.name}: ${fmtK(p.monthly_spend||0)} AUM · ${fmt$((p.monthly_spend||0)*REV_RATE)} comp/mo`)} style={{fontSize:12,padding:"6px 12px"}}><i className="ti ti-brand-slack" style={{fontSize:12,marginRight:5}}/>Notify Slack</button>
              </div>
            </div>}
          </Card>;
        })}
      ...
      {(view==="admin-brands"||view==="admin-agencies")&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 20px",fontSize:18,fontWeight:500}}>{view==="admin-brands"?"All brand clients":"All agency clients"}</h2>
        {pipeline.filter(c=>c.type===(view==="admin-brands"?"brand":"agency")).length===0
          ?<EmptyState icon={view==="admin-brands"?"building":"briefcase"} title="No clients yet" sub="Partners will add clients from their dashboard."/>
          :<Card style={{overflow:"hidden"}}>
            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",padding:"10px 16px",background:C.bg2,borderBottom:`0.5px solid ${C.border}`,fontSize:11,fontWeight:500,color:C.muted}}><span>Name</span><span>Spend</span><span>Stage</span><span>Partner</span><span>Added</span></div>
            {pipeline.filter(c=>c.type===(view==="admin-brands"?"brand":"agency")).map((c,i,arr)=><div key={c.id} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",padding:"12px 16px",borderBottom:i<arr.length-1?`0.5px solid ${C.border}`:"none",alignItems:"center"}}>
              <div><div style={{fontWeight:500,fontSize:13}}>{c.name}</div><div style={{fontSize:11,color:C.muted}}>{c.contact_name}</div></div>
              <div style={{fontSize:13,fontWeight:500,color:C.green}}>{fmtK(c.monthly_spend||0)}</div>
      ...
      {view==="approvals"&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 4px",fontSize:18,fontWeight:500}}>Client approvals</h2>
        <div style={{fontSize:12,color:C.muted,marginBottom:20}}>Clients in Review stage — pending approval</div>
        {pendingApprovals.length===0?<EmptyState icon="check-circle" title="No pending approvals" sub="Clients submitted by partners in Review stage will appear here."/>
        :pendingApprovals.map(c=><Card key={c.id} style={{padding:14,marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div style={{flex:1}}><div style={{fontWeight:500,fontSize:13}}>{c.name}</div><div style={{fontSize:11,color:C.muted}}>Partner: {c.partner_name} · {fmtK(c.monthly_spend||0)}/mo</div></div>
            <button onClick={()=>movePipelineStage(c.id,"Approved")} style={{fontSize:12,padding:"6px 12px",background:C.greenBg,color:C.green,border:`0.5px solid #C0DD97`,borderRadius:8,cursor:"pointer"}}>Approve</button>
            <button onClick={()=>movePipelineStage(c.id,"Lead")} style={{fontSize:12,padding:"6px 12px",background:C.redBg,color:C.red,border:`0.5px solid #F7C1C1`,borderRadius:8,cursor:"pointer"}}>Reject</button>
          </div>
        </Card>)}
      </div>}

      {view==="comp-tracking"&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 4px",fontSize:18,fontWeight:500}}>Compensation tracking</h2>
        <div style={{fontSize:12,color:C.muted,marginBottom:20}}>Ad Revenue Under Management · 10% rev share · DSP Connect Stripe account</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
          <StatBox label="Total monthly AUM" value={fmtK(totalAUM)} icon="chart-bar"/>
          <StatBox label="Monthly comp due" value={fmt$(totalComp)} icon="currency-dollar" green/>
          <StatBox label="Annual run rate" value={fmtK(totalComp*12)} icon="trending-up" green/>
        </div>
        {partners.length===0?<EmptyState icon="currency-dollar" title="No partners yet" sub="Compensation will appear here as partners are added."/>
        :partners.map(p=><Card key={p.id} style={{marginBottom:12,overflow:"hidden"}}>
          <div style={{padding:"12px 16px",borderBottom:`0.5px solid ${C.border}`,display:"flex",alignItems:"center",gap:12}}>
            <Avatar name={p.name} size={36}/><div style={{flex:1}}><div style={{fontWeight:500,fontSize:13}}>{p.name}{p.agency?` · ${p.agency}`:""}</div><div style={{fontSize:11,color:C.muted}}>{p.vertical}</div></div>
            <StatusPill s={p.status}/><div style={{fontSize:13,fontWeight:500,color:C.green}}>10% · {fmt$((p.monthly_spend||0)*REV_RATE)}/mo</div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)"}}>
            {[{l:"Monthly AUM",v:fmtK(p.monthly_spend||0)},{l:"Annual AUM",v:fmtK(p.annual_aum||0)},{l:"Monthly comp",v:fmt$((p.monthly_spend||0)*REV_RATE),green:true},{l:"Annual comp",v:fmtK((p.annual_aum||0)*REV_RATE),green:true}].map((m,i)=><div key={m.l} style={{padding:"10px 16px",borderRight:i<3?`0.5px solid ${C.border}`:"none"}}><div style={{fontSize:11,color:C.muted,marginBottom:3}}>{m.l}</div><div style={{fontSize:15,fontWeight:500,color:m.green?C.green:C.text}}>{m.v}</div></div>)}
          </div>
        </Card>)}
      </div>}

      {view==="stripe"&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 4px",fontSize:18,fontWeight:500}}>Stripe payouts</h2>
        <div style={{fontSize:12,color:C.muted,marginBottom:20}}>Disbursements from DSP Connect Stripe · Partners do not connect their own Stripe</div>
        <div style={{background:C.greenBg,border:"0.5px solid #C0DD97",borderRadius:10,padding:"10px 16px",marginBottom:20,fontSize:13,color:C.green,display:"flex",alignItems:"center",gap:10}}>
          <i className="ti ti-check-circle" style={{fontSize:16}}/>DSP Connect Stripe account · All payouts flow from your account only
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
          <StatBox label="Next payout batch" value={fmt$(totalComp)} sub="All active partners" icon="send" green/>
          <StatBox label="Active partners" value={active.length} icon="users"/>
          <StatBox label="Payout rate" value="10% AUM" icon="percentage"/>
        </div>
        {active.map(p=><Card key={p.id} style={{marginBottom:10,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
          <Avatar name={p.name} size={36}/><div style={{flex:1}}><div style={{fontWeight:500,fontSize:13}}>{p.name}{p.agency?` · ${p.agency}`:""}</div><div style={{fontSize:11,color:C.muted}}>{p.email}</div></div>
          <div style={{fontSize:13,fontWeight:500,color:C.green}}>10% · {fmt$((p.monthly_spend||0)*REV_RATE)}/mo</div>
          <div style={{fontSize:12,color:C.muted}}>{fmtK(p.monthly_spend||0)} AUM</div>
        </Card>)}
      </div>}

      {view==="content-mgmt"&&<div style={{padding:24}}>
        <SectionHead title="Content management" action={<button onClick={()=>setShowAddResource(s=>!s)} style={{fontSize:13,padding:"8px 16px",background:showAddResource?"transparent":C.blue,color:showAddResource?C.text:"#fff",border:showAddResource?`0.5px solid ${C.border}`:"none",borderRadius:8,cursor:"pointer"}}>{showAddResource?"Cancel":"+ Add resource"}</button>}/>
        {showAddResource&&<Card style={{padding:16,marginBottom:16}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div><label style={{fontSize:12,color:C.muted,display:"block",marginBottom:4}}>Title *</label><input value={newResource.title} onChange={e=>setNewResource(r=>({...r,title:e.target.value}))} style={{width:"100%",boxSizing:"border-box"}}/></div>
            <div><label style={{fontSize:12,color:C.muted,display:"block",marginBottom:4}}>Type</label><select value={newResource.type} onChange={e=>setNewResource(r=>({...r,type:e.target.value}))} style={{width:"100%",boxSizing:"border-box"}}>{["pdf","video","pptx","docx","article"].map(t=><option key={t}>{t}</option>)}</select></div>
            <div><label style={{fontSize:12,color:C.muted,display:"block",marginBottom:4}}>Category</label><select value={newResource.category} onChange={e=>setNewResource(r=>({...r,category:e.target.value}))} style={{width:"100%",boxSizing:"border-box"}}>{["training","sops","documents"].map(c=><option key={c}>{c}</option>)}</select></div>
            <div><label style={{fontSize:12,color:C.muted,display:"block",marginBottom:4}}>Description</label><input value={newResource.description} onChange={e=>setNewResource(r=>({...r,description:e.target.value}))} style={{width:"100%",boxSizing:"border-box"}}/></div>
          </div>
          <div style={{background:C.bg2,border:`0.5px dashed ${C.border2}`,borderRadius:8,padding:20,textAlign:"center",marginBottom:10}}>
            <i className="ti ti-upload" style={{fontSize:24,color:C.muted,display:"block",marginBottom:8}}/>
            <div style={{fontSize:13,color:C.muted}}>File upload available — add URL after deploy</div>
          </div>
          <div style={{display:"flex",gap:8}}><button onClick={addResource} style={{fontSize:13,padding:"7px 14px",background:C.blue,color:"#fff",border:"none",borderRadius:8,cursor:"pointer"}}>Add resource</button><button onClick={()=>setShowAddResource(false)} style={{fontSize:13,padding:"7px 14px"}}>Cancel</button></div>
        </Card>}
        {["training","sops","documents"].map(cat=><div key={cat} style={{marginBottom:20}}>
          ...
          {resources.filter(r=>r.category===cat).length===0?<div style={{fontSize:13,color:C.muted}}>None yet.</div>
          :resources.filter(r=>r.category===cat).map(r=><Card key={r.id} style={{padding:"12px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}><div style={{fontWeight:500,fontSize:13}}>{r.title}</div><div style={{fontSize:11,color:C.muted}}>{r.type?.toUpperCase()} · {r.description}</div></div>
          </Card>)}
        </div>)}
      </div>}

      {view==="slack"&&<div style={{padding:24}}>
        <h2 style={{margin:"0 0 20px",fontSize:18,fontWeight:500}}>Slack</h2>
        <Card style={{padding:20,marginBottom:14}}>
          <div style={{fontWeight:500,fontSize:13,marginBottom:12}}>Message log</div>
          <div style={{background:C.bg2,borderRadius:10,padding:12,maxHeight:240,overflowY:"auto",display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
            {slackMsgs.length===0&&<div style={{color:C.muted,fontSize:13,textAlign:"center",padding:"2rem 0"}}>No notifications yet.</div>}
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>{if(slackMsg.trim()){sendSlack(slackCh,slackMsg);setSlackMsg("");}}} style={{padding:"6px 14px"}}>Send</button>
          </div>
        </Card>
        <Card style={{padding:20}}>
          <div style={{fontWeight:500,fontSize:13,marginBottom:12}}>Quick notifications</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
            {[{label:"Monthly comp summary",msg:`💰 Monthly summary: ${fmtK(totalAUM)} total AUM · ${fmt$(totalComp)} partner compensation due · ${active.length} active partners`},{label:"Payout reminder",msg:`📋 Process monthly payouts via DSP Connect Stripe. ${active.length} partners · ${fmt$(totalComp)} total.`},{label:"Equity reminder",msg:`📊 Equity reminder: $5M annual AUM minimum required for vesting.`}].map(q=><button key={q.label} onClick={()=>sendSlack(slackCh,q.msg)} style={{fontSize:12,padding:"7px 14px"}}>{q.label}</button>)}
          </div>
        </Card>
      </div>}
